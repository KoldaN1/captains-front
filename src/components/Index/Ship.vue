...
<template>
  <div class="flex justify-center items-center p-4 relative h-[280px]">
    ...
    <div v-else class="font-medium flex flex-col items-center gap-y-4">
      ...
      <button @click="emit('getInitialShip')" class="bg-yellow-500 animate-pulse w-fit active:opacity-75 transition-all px-4 py-2.5 rounded-xl flex shadow-sm uppercase font-semibold text-white">
        {{ $t("get_free") }}
      </button>
    </div>
    ...
    <div v-if="theme === 'dark'" class="stars opacity-50">
      <img
        v-for="n in 10"
        :key="n"
        :style="[getRandomPosition(), { animationDelay: getRandomDelay(), width: getRandomSize(10, 20), height: getRandomSize(10, 20) }]"
        src="../../assets/star.svg"
        class="star"
      />
