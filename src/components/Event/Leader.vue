...
const avatarColor = stringToColor(
  props.user?.users?.username?.slice(0, 2) || props.user?.users?.name?.slice(0, 2) || "",
);
  ...
  <div class="flex text-sm justify-between bg-bg_color py-2 px-3 rounded-2xl items-center">
    <div class="flex items-center">
      ...
      <span
        ...
        class="w-8 aspect-square mr-2.5 rounded-xl flex justify-center items-center font-medium uppercase"
        >{{ user?.users?.name?.slice(0, 2) || user?.users?.username?.slice(0, 2) || "AN" }}</span
      >
      <span class="font-semibold truncate max-w-[112px]">{{
        user?.users?.name || user?.users?.username || "Anon"
      }}</span>
    </div>
